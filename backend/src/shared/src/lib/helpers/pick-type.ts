...
export function PickType<T extends new (...args: any[]) => any, K extends keyof InstanceType<T>>(
  BaseClass: T,
  keys: readonly K[]
) {
  class PickClass {}
  ...
  for (const key of keys) {
    const type = Reflect.getMetadata('design:type', BaseClass.prototype, key as string);
    Reflect.defineMetadata('design:type', type, PickClass.prototype, key as string);
  }
